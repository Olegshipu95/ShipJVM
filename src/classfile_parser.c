...
int parse_attributes(Loader* loader, struct class_file* class,
  ...
  attr->info =
      malloc((size_t)(attr->attribute_length) * sizeof(struct attribute_info));
...
int parse_class_fields(Loader* loader, struct field_info* fields) {
  ...
  int err =
      parse_attributes(loader, fields->attributes_count, fields->attributes);
  return err;
}
